...
{
    internal const string Scheme = "WorkIqSignIn";
    private const string CookieScheme = "WorkIqCookies";

    internal static void AddBrowserSignIn(
        this IServiceCollection services, IConfiguration configuration, Uri publicUrl)
    {
        string redirectUri = new Uri(publicUrl, configuration["WorkIQ:CallbackPath"] ?? "/signin-workiq").AbsoluteUri;
        services.AddMemoryCache();
        services.AddSingleton<SignInStore>();
        services.AddAuthentication()
            .AddMicrosoftIdentityWebApp(configuration.GetSection("WorkIQ"),
                openIdConnectScheme: Scheme, cookieScheme: CookieScheme)
            .EnableTokenAcquisitionToCallDownstreamApi(WorkIqService.Scopes)
